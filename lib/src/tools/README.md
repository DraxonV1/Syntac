...
`agent_tools.dart` exposes sandboxed `read`, `write`, `edit`, `delete`, `list`, `search`, and `bash` operations.
...
- Keep Bash output at 50 preview lines and bounded characters. Persist full bounded output under project `.omp/agent/blobs/` and return `local://` reference when preview truncates.

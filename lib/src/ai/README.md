...
`registry/` controls supported providers, capabilities, beta visibility, and defaults. `oauth/` owns login/refresh/discovery. `auth/` owns credential access.
...
3. Add protocol fixtures for streaming text, tool calls, reasoning, malformed data, auth failures, and cancellation.
4. Sanitize all user-facing errors and run focused provider tests before full validation.

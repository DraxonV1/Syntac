...
- `RuntimeJobSupervisor.kt`: persistent Arch job registry, app-private bounded logs, process ownership, restart/status/log/stop APIs, and crash recovery state.
...
- `cancelCommand` and job stop APIs must kill active process trees.
